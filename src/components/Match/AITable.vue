...
import AITableVariant from './AITableVariant.vue';
...
<template>
    ...
    <div v-if="state.matches('player1TurnLimited') || state.matches('player2TurnLimited')" :class = limited_table_variant_style>
        <AITableVariant :player = player :state = state :send = send :limited = true :service = service />
    </div>

    <div v-else-if="state.matches(player + 'Turn')" :class = full_control_table_variant_style>
        <AITableVariant :player = player :state = state :send = send :service = service />
    </div>
